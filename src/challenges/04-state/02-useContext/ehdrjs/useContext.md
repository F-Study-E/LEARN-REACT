...
**커스텀 Provider 쓸 때:** state가 `CountProvider` 안에 있다. App에는 state가 없으니 App은 리렌더 안 된다. App이 안 돌면 `<A /><B />`라는 JSX도 새로 안 만들어지고, children 참조가 그대로라서 B는 스킵된다.
...
이게 **memo를 뚫는 이유**다. `React.memo`는 부모 → 자식 리렌더 경로에서 props가 같으면 스킵하는 로직인데, Context propagation은 그 경로를 안 탄다. Fiber에 직접 lane을 꽂기 때문에 memo 검사를 거치지 않는다.
...
사실 "올라가며 찾는다"고 했지만, 실제로는 Context 객체 자체에 현재 value가 저장되어 있어서(`context._currentValue`) 순회 없이 바로 읽는다. Provider가 렌더링될 때 이 값을 갱신해두기 때문.
